...
def find_spaces(sentence):
    ...
    n = len(sentence)
    spaces = [True] * n  # Pre-allocate with default value

    # Pre-define the sets (faster than recreation and tuple lookup each time)
    punctuations = {',', '.', '!', '?', ')', ':', ';', '”', '…', '...', '/', '%'}
    leading_no_space = {'(', '“', '/'}
    quote_char = '"'

    # Precompute last indexes (avoid recomputation)
    n_minus_1 = n - 1
    n_minus_2 = n - 2
    n_minus_3 = n - 3

    # Avoid redundant len(sentence) checks in the loop
    for word_idx in range(n):
        word = sentence[word_idx]
        space = True

        # Combined next-word lookups, avoid repeated bounds checks
        if word_idx < n_minus_2 and sentence[word_idx+1] == quote_char:
            next2 = sentence[word_idx+2]
            if next2 == '.':
                space = False
            elif word_idx == n_minus_3 and next2 == '...':
                space = False

        if word_idx < n_minus_1:
            if sentence[word_idx+1] in punctuations:
                space = False

        if word in leading_no_space:
            space = False
